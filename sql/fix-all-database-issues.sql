-- Comprehensive fix for all database constraint and permission issues
-- Run this SQL in your Supabase SQL Editor to fix all problems

-- 1. First, let's make ALL potentially problematic columns nullable
-- This prevents NOT NULL constraint violations

-- Handle drivers table constraints
ALTER TABLE drivers 
...
-- 2. Add any missing columns that the app expects (all nullable)
ALTER TABLE drivers 
...
-- 3. Ensure vehicles table has all needed columns
ALTER TABLE vehicles 
ADD COLUMN IF NOT EXISTS image TEXT,
ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 4. Ensure bookings table has all needed columns
...
-- 5. Create earnings table if it doesn't exist
...
-- 7. Drop ALL existing policies and create very permissive ones
-- Drivers policies
...
-- 8. Clean up any existing test data and recreate
...
-- 9. Insert comprehensive test driver data with ALL possible required fields
INSERT INTO drivers (
  id, 
  name, 
  phone, 
...
-- 14. Verify the data was inserted correctly
...
-- 15. Show the final driver record to verify all fields
